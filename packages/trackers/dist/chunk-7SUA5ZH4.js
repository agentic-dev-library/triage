...
var JiraProvider = class {
  ...
  async createIssue(options) {
    const body = {
      fields: {
        ...
        description: options.description ? this.stringToADF(options.description) : void 0,
        issuetype: { name: this.mapTypeToJira(options.type || "task") },
        priority: options.priority ? { name: this.mapPriorityToJira(options.priority) } : void 0,
  ...
  async closeIssue(id, _reason) {
    return this.updateIssue(id, { status: "closed" });
  }
  async reopenIssue(id, _reason) {
  ...
  async listIssues(options) {
    ...
    return (result.issues || []).map((i) => this.mapJiraIssue(i));
  }
  ...
  async transitionIssue(id, status) {
    ...
    const transition = transitions.transitions.find(
      (t) => t.name.toLowerCase() === jiraStatus.toLowerCase() || t.to.name.toLowerCase() === jiraStatus.toLowerCase()
    );
    if (transition) {
  ...
  adfToString(adf) {
    if (!adf || !adf.content) return void 0;
    let text = "";
